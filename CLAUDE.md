...
- Plain AppKit, compiled with `swiftc` by `build.sh` (tests → build `.app` → copy to `~/Applications` → LaunchAgent `local.claude-usage` → launch).
...
- Launched by the LaunchAgent `~/Library/LaunchAgents/local.claude-usage.plist` (`KeepAlive` on unsuccessful exit): crashes restart, but Quit exits 0 and stays quit until next login. Don't also add a login item — it would start a second copy.
